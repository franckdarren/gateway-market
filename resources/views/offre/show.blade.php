<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-[#0D062D]">
            ...
            <div class="p-6 bg-white rounded-lg shadow-lg sm:p-8">
                <div class="text-black">
                    ...
                    <div class="space-y-4 ">
                        ...
                        <div class="flex gap-5 w-full max-w-[300px] items-center justify-center p-4 text-black bg-[#cfdfea] rounded-md">
                            <h3 class="text-sm font-semibold">Statut :</h3>
                            <p class="text-sm">{{ $offre->statut }}</p>
                        </div>
                        <div class="flex flex-col gap-1 md:grid md:grid-cols-2 xl:flex xl:flex-row  py-4 justify-between">

                            <div class="flex gap-1 text-black">
                                <h3 class="text-sm font-semibold">Investissement :</h3>
                                <p class="text-sm text-[#5030E5] font-bold">
                                    {{ number_format($offre->montant, 0, ',', ' ') }} FCFA
                                </p>
                            </div>
                            <div class="flex gap-1 text-black">
                                <h3 class="text-sm font-semibold">Gain :</h3>
                                <p class="text-sm text-[#088606] font-bold">
                                    {{ number_format($offre->montant, 0, ',', ' ') }} FCFA
                                </p>
                            </div>
                            <div class="flex gap-1">
                        ...
                        <table class="w-full text-left border border-collapse border-gray-300 table-auto  lg:hidden">
                            <thead class="w-full">
                                <tr class="bg-gray-200">
                                    <th
                                        class="px-2 py-2 text-sm font-semibold  text-white border border-gray-300 sm:px-4">
                                        VAN</th>
                                    <th
                                        class="px-2 py-2 text-sm font-semibold  text-white border border-gray-300 sm:px-4">
                                        IR</th>
                    ...
                    <div class="flex flex-col flex-wrap gap-4 sm:flex-row">
                        @if ($offre->url_business_plan)
                            <a href="{{ Storage::url($offre->url_business_plan) }}" target="_blank"
                                class="flex bg-[#5B3FFC] items-center justify-center gap-2 px-4 py-2 text-center text-white rounded-md shadow hover:bg-[#5B3FFC]/80">
                        ...
                        @if ($offre->url_etude_risque)
                            <a href="{{ Storage::url($offre->url_etude_risque) }}" target="_blank"
                                class="flex items-center bg-[#5B3FFC] justify-center gap-2 px-4 py-2 text-center text-white rounded-md shadow hover:bg-[#5B3FFC]/80">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.03906 12.3178L8.53334 9.07598L11.3785 11.3109L13.8194 8.16064"
                                        stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                    <circle cx="16.6643" cy="3.50027" r="1.60183" stroke="white" stroke-width="1.5"
                    ...
                    @if ($offre->compte_investisseur_id !== $compteInvestisseurId)
                        <div x-data="{ showModal: false }">
                            ...
                            <a href="javascript:void(0)" @click="showModal = true"
                                class="px-5 py-2.5 bg-[#5B3FFC] hover:bg-[#5B3FFC]/90 text-white rounded-md text-center">
                                Investir
                            </a>
